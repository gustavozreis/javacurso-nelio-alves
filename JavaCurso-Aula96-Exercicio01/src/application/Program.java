...
public class Program {
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<eReg; i++) {
			...
			System.out.print("Enter employee salary: "); // salário
			double salary = sc.nextDouble();
			
			System.out.println("");
			
			//instanciação e inserir funcionários na lista
			regEmployees.add(new Employee(id, name, salary));
			
		}
		
		//pedir input do aumento de salário
		System.out.print("Enter the employee id that will have a salary increase: ");
		int eId = sc.nextInt();
		
		//procurar pelo usuário na lista pelo número de id
		Employee empTemp = regEmployees.stream().filter(x -> x.getEmpId() == eId).findFirst().orElse(null);
		
		if (empTemp == null) {
			System.out.println("Esse id de usuário não existe!"); // erro - id não existe
		} else {
